...
What it does
Accepts multiple files at once — drop in any combination of PDFs, Excel (.xlsx / .xls), and CSV files.
Detects email and name columns automatically — works regardless of how the column is labelled in your spreadsheet.
Extracts every email it can find — including multiple emails stuffed into a single cell, and any email address embedded anywhere in a PDF document.
Cleans and validates — strips whitespace, invisible characters, and filters out anything that isn't a real email address.
Removes duplicates — even across multiple files. Each email address appears only once in the output.
Lets you preview, search, and sort results in a table before downloading.
Exports to Excel (.xlsx) or CSV with exactly two columns: name and email.
...
How to use it
Drop files onto the upload zone, or click it to browse. You can select multiple files at once.
Review the file queue — each file shows its type, name, and size. Remove any you don't want with the ✕ button.
Click "Extract Emails from X files" to start processing.
View the statistics panel to see how many emails were found, how many duplicates were removed, and how many invalid entries were skipped.
Search or sort the results table. A Source column shows which file each email came from.
Click Download CSV or Download Excel (.xlsx) to save the cleaned list.
...
Recognised name column headers: Name, Full Name, Contact Name, First Name, Last Name, Person, Client, Lead

Recognised email column headers: Email, Emails, Email Address, Contact Email, E-mail
